/* ============================================================
   ...
   viruses, missions, unlock gates, names & flavor.
...
/* ============================================================
   TRAITS — heritable passives. `ico` is a pixel-atlas name.
   ============================================================ */
DATA.TRAITS = {
  dank:       { name: 'Dank',            ico: 'fire',      kind: 'good', desc: '+15% crit chance. Certified.' },
  wholesome:  { name: 'Wholesome',       ico: 'heart',     kind: 'good', desc: 'Regenerates 2 HP at the start of its turn.' },
  viral:      { name: 'Viral',           ico: 'virus',     kind: 'good', desc: 'On kill: deals 3 dmg to enemies next to the victim.' },
  ...
  blessed:    { name: 'Blessed',         ico: 'halo',      kind: 'good', desc: '12% chance to completely dodge attacks.' },
  ...
  cringe:     { name: 'Cringe',          ico: 'gritface',  kind: 'bad',  desc: 'Adjacent allies deal -2 damage (secondhand embarrassment).' },
  ...
  clickbait:  { name: 'Clickbait',       ico: 'cursor',    kind: 'weird',desc: "Viruses can't resist attacking this meme first." },
  cursed:     { name: 'Cursed',          ico: 'eye',       kind: 'weird',desc: '6% chance to bonk itself when attacking. Why.' },
...
DATA.ABILITIES = {
  bonk: {
    name: 'BONK', ico: 'hammer', cd: 0, range: 1, target: 'enemy', stat: 'atk', power: 1.0,
    desc: 'Classic. Bonk an adjacent virus for ATK damage.'
  },
  yeet: {
    name: 'YEET', ico: 'throw', cd: 2, range: 1, target: 'enemy', stat: 'atk', power: 1.2, knockback: 2,
    desc: 'Bonk a virus and YEET it 2 tiles backwards.'
  },
  deepfry: {
    name: 'Deep Fry', ico: 'fries', cd: 2, range: 3, target: 'enemy', stat: 'int', power: 1.2,
    status: { id: 'burn', turns: 2, chance: 0.9 },
    desc: 'Hurl sizzling fryer oil. Deals BRAIN damage and sets the target on fire.'
  },
  rickroll: {
    name: 'Rickroll', ico: 'note', cd: 3, range: 3, target: 'enemy', stat: 'int', power: 0.3,
    status: { id: 'confuse', turns: 2, chance: 1 },
    desc: 'Never gonna give you up. Confuses the target — it acts randomly for 2 turns.'
  },
  copypasta: {
    name: 'Copypasta', ico: 'clipboard', cd: 6, range: 0, target: 'self', stat: 'int', power: 0, summon: true,
    desc: 'Paste a cheap clone of yourself into an adjacent tile (40% stats, expires with the battle).'
  },
  touchgrass: {
    name: 'Touch Grass', ico: 'sprout', cd: 3, range: 0, target: 'self', stat: 'int', power: 1.2, heal: true, aoe: 1,
    desc: 'Go outside for a second. Heals you and adjacent allies for BRAIN x1.2.'
  },
  stonks: {
    name: 'STONKS', ico: 'chartup', cd: 3, range: 2, target: 'ally', stat: 'int', power: 0,
    status: { id: 'atkUp', turns: 3, chance: 1 },
    desc: 'Line goes up. An ally gains +50% damage for 3 turns.'
  },
  sadviolin: {
    name: 'Sad Violin', ico: 'violin', cd: 3, range: 3, target: 'enemy', stat: 'int', power: 0.2, aoe: 1,
    status: { id: 'atkDown', turns: 2, chance: 1 },
    desc: "Plays the world's smallest violin. Enemies in the area deal -40% damage for 2 turns."
  },
  unoreverse: {
    name: 'UNO Reverse', ico: 'loop', cd: 4, range: 0, target: 'self', stat: 'int', power: 0,
    selfStatus: { id: 'reflect', turns: 2 },
    desc: 'No u. The next hit you take is reflected back at the attacker.'
  },
  vibecheck: {
    name: 'Vibe Check', ico: 'hand', cd: 3, range: 0, target: 'self', stat: 'atk', power: 0.9, aoe: 1, hitsEnemiesOnly: true,
  ...
  ratio: {
    name: 'Ratio', ico: 'bubble', cd: 2, range: 4, target: 'enemy', stat: 'int', power: 1.0, fullhp: 1.5,
    desc: 'L + ratio. Ranged BRAIN damage; +50% vs viruses at full HP.'
  },
  zoomies: {
    name: 'Zoomies', ico: 'wind', cd: 3, range: 0, target: 'self', stat: 'atk', power: 0, extraMove: true,
    selfStatus: { id: 'spdUp', turns: 2 },
    desc: '3 AM energy. Move again this turn and gain +2 move for 2 turns.'
  },
  banhammer: {
    name: 'Ban Hammer', ico: 'hammer', cd: 4, range: 1, target: 'enemy', stat: 'atk', power: 1.8,
    status: { id: 'stun', turns: 1, chance: 0.8 },
    desc: 'MODS?! Massive damage + likely stun. Straight to horny jail.'
  },
  thoughtsprayers: {
    name: 'Thoughts & Prayers', ico: 'pray', cd: 2, range: 3, target: 'ally', stat: 'int', power: 2.0, heal: true, lucky: true,
    desc: 'Sends support. LUCK% chance of a HUGE heal... otherwise heals 1. One (1) HP.'
  },
  airhorn: {
    name: 'MLG Airhorn', ico: 'horn', cd: 3, range: 3, target: 'enemy', stat: 'int', power: 0.8, line: true,
    desc: 'BWAAAAMP. Blasts a straight line, hitting every virus in it.'
  },
  nyandash: {
    name: 'Nyan Dash', ico: 'rainbow', cd: 3, range: 3, target: 'enemy', stat: 'atk', power: 0.9, dash: true,
    desc: 'Rainbow-dash in a straight line at a virus, leaving sparkles. Ends adjacent to it.'
  },
  cheems: {
    name: 'Cheemsburbger', ico: 'burger', cd: 4, range: 0, target: 'self', stat: 'int', power: 1.5, heal: true,
    selfStatus: { id: 'atkUp', turns: 2 },
    desc: 'Eat a burbger. Big self-heal and +50% damage for 2 turns.'
  },
  ggez: {
    name: 'GG EZ', ico: 'trophy', cd: 2, range: 1, target: 'enemy', stat: 'atk', power: 1.0, execute: 0.35,
...
/* ============================================================
   ...
   ============================================================ */
DATA.STATUS = {
  burn:    { name: 'Deep-Fried', ico: 'fire',      bad: true,  desc: 'Takes 3 damage at the start of its turn.' },
  poison:  { name: 'Malware',    ico: 'drop',      bad: true,  desc: 'Takes 2 damage at the start of its turn.' },
  stun:    { name: 'Bonked',     ico: 'dizzy',     bad: true,  desc: 'Skips its next turn. Go to sleep.' },
  confuse: { name: 'Rickrolled', ico: 'note',      bad: true,  desc: 'Acts randomly. Never gonna give you up...' },
  slow:    { name: 'Lagging',    ico: 'snail',     bad: true,  desc: '-2 movement. 300 ping.' },
  atkUp:   { name: 'STONKS',     ico: 'arrowup',   bad: false, desc: '+50% damage dealt.' },
  atkDown: { name: 'Not Stonks', ico: 'arrowdown', bad: true,  desc: '-40% damage dealt.' },
  spdUp:   { name: 'Zoomin',     ico: 'wind',      bad: false, desc: '+2 movement.' },
  reflect: { name: 'No U',       ico: 'loop',      bad: false, desc: 'Reflects the next hit back at the attacker.' },
  shield:  { name: 'Blocked',    ico: 'shield',    bad: false, desc: 'Absorbs incoming damage.' },
};

/* ============================================================
   ITEMS — ico names double as the equipped pixel sprite.
   ============================================================ */
DATA.ITEMS = {
  // ---- hats ----
  tophat:    { name: 'Like a Sir',      ico: 'tophat',    kind: 'hat', price: 40, stats: { int: 2 },           desc: '+2 BRAIN. Tips fedora.' },
  mlgcap:    { name: 'MLG Snapback',    ico: 'cap',       kind: 'hat', price: 45, stats: { crit: 10 },          desc: '+10% crit. 360 no-scope certified.' },
  crown:     { name: 'Crown of Sheesh', ico: 'crown',     kind: 'hat', price: 120, stats: { hp: 4, atk: 1, int: 1, spd: 1, lck: 1 }, desc: '+1 to everything, +4 HP. Royalty behavior.' },
  propeller: { name: 'Propeller Cap',   ico: 'propeller', kind: 'hat', price: 45, stats: { spd: 2 },            desc: '+2 ZOOM. Nyoooom.' },
  tinfoil:   { name: 'Tinfoil Hat',     ico: 'tinfoil',   kind: 'hat', price: 50, stats: { resist: 30 },        desc: "30% chance to resist debuffs. They can't read your thoughts now." },
  partyhat:  { name: 'Party Hat',       ico: 'partyhat',  kind: 'hat', price: 40, stats: { lck: 5 },            desc: "+5 LUCK. It's always somebody's birthday." },
  // ---- held ----
  banhammeritem: { name: 'Spare Ban Hammer', ico: 'hammer', kind: 'held', price: 60, stats: { atk: 3 },      desc: '+3 BONK. For legal reasons this is a joke.' },
  keyboard:  { name: 'Mechanical Keyboard', ico: 'keyboard', kind: 'held', price: 55, stats: { atk: 2, int: 1 }, desc: '+2 BONK, +1 BRAIN. Clacky.' },
  dogecoin:  { name: 'Doge Coin',       ico: 'coin',      kind: 'held', price: 50, stats: { lck: 6 },           desc: '+6 LUCK. Much fortune. Wow.' },
  gpu:       { name: 'Hot GPU',         ico: 'gpu',       kind: 'held', price: 65, stats: { int: 3 },           desc: '+3 BRAIN. Runs Crysis (barely).' },
  gamermouse:{ name: 'Gamer Mouse',     ico: 'mouse',     kind: 'held', price: 55, stats: { spd: 2, crit: 5 },  desc: '+2 ZOOM, +5% crit. RGB makes it faster.' },
  popblocker:{ name: 'Pop-up Blocker',  ico: 'shield',    kind: 'held', price: 60, stats: { hp: 10 },           desc: '+10 max HP. NOT TODAY, ADS.' },
  // ---- consumables ----
  pizza:     { name: 'Pizza Slice',     ico: 'pizza',      kind: 'consumable', price: 15, battle: 'heal', power: 18, desc: 'BATTLE: heal a meme for 18 HP.' },
  energy:    { name: 'G-Fuel Barrel',   ico: 'energycan',  kind: 'consumable', price: 20, battle: 'energy', desc: 'BATTLE: the current meme can move again & loses ability cooldowns.' },
  copium:    { name: 'Tank of Copium',  ico: 'copiumtank', kind: 'consumable', price: 80, battle: 'revive', desc: 'BATTLE: revive a fallen meme at 50% HP. Pure copium.' },
  usbstick:  { name: 'Antivirus USB',   ico: 'usb',        kind: 'consumable', price: 70, battle: 'nuke', power: 10, desc: 'BATTLE: deals 10 damage to EVERY virus.' },
  ...
  miraclegro:{ name: 'Miracle-Gro',     ico: 'flower',     kind: 'consumable', price: 35, home: 'grow', desc: 'HOME: instantly grows a baby meme into an adult.' },
  dankserum: { name: 'Serum of Dank',   ico: 'syringe',    kind: 'consumable', price: 100, home: 'trait', desc: 'HOME: injects a random GOOD trait into a meme.' },
  febreze:   { name: 'Meme Febreze',    ico: 'spray',      kind: 'consumable', price: 90, home: 'cleanse', desc: 'HOME: removes a random BAD trait from a meme.' },
};

/* ============================================================
   VIRUSES (enemies) — `art` is the pixel sprite key (auto-set
   to the virus id in sprites.js).
   ai: 'melee' | 'ranged' | 'summoner' | 'coward' | 'miner'
   ============================================================ */
DATA.VIRUSES = {
  popup: {
    name: 'Pop-Up Ad', hp: 8, atk: 3, spd: 4, move: 3, ai: 'melee',
    xp: 4, bounty: 3, abilities: ['vbite'],
    flavor: 'CONGRATULATIONS!! You are the 1,000,000th victim.'
  },
  worm: {
    name: 'Worm.exe', hp: 12, atk: 4, spd: 7, move: 4, ai: 'melee',
    xp: 6, bounty: 5, abilities: ['vpoison'],
    flavor: 'It wriggles through your files leaving slime and malware.'
  },
  trojan: {
    name: 'Trojan Pony', hp: 30, atk: 7, spd: 3, move: 2, ai: 'melee',
    xp: 12, bounty: 10, abilities: ['vbite'], tanky: true,
    flavor: 'It said it was a free game. IT SAID IT WAS A FREE GAME.'
  },
  blob: {
    name: 'Bloatware Blob', hp: 26, atk: 5, spd: 2, move: 2, ai: 'melee',
    xp: 10, bounty: 8, abilities: ['vbite'], splits: true,
    flavor: 'Ships pre-installed. Cannot be uninstalled. Splits when deleted.'
  },
  miniblob: {
    name: 'Blob Jr.', hp: 8, atk: 3, spd: 4, move: 3, ai: 'melee',
    xp: 3, bounty: 2, abilities: ['vbite'],
    flavor: 'A smaller, angrier terms-of-service agreement.'
  },
  spyder: {
    name: 'Keylogger Spyder', hp: 14, atk: 5, spd: 6, move: 3, ai: 'ranged',
    xp: 9, bounty: 8, abilities: ['vweb', 'vsnipe'],
    flavor: 'It knows you typed "how to delete virus" 47 times.'
  },
  ransom: {
    name: 'Ransom-Where', hp: 22, atk: 6, spd: 5, move: 3, ai: 'melee',
    xp: 12, bounty: 12, abilities: ['vencrypt'],
    flavor: 'Your files are encrypted! Send 3 dogecoins to unlock.'
  },
  phish: {
    name: 'Phish', hp: 16, atk: 5, spd: 5, move: 3, ai: 'ranged',
    xp: 10, bounty: 9, abilities: ['vphish', 'vbite'],
    flavor: 'Dear Sir/Madam, you have won a hook to the face.'
  },
  drone: {
    name: 'Botnet Drone', hp: 10, atk: 4, spd: 8, move: 4, ai: 'melee',
    xp: 5, bounty: 4, abilities: ['vbite'],
    flavor: 'One of ten thousand. All named Kevin.'
  },
  adware: {
    name: 'Adware Broadcaster', hp: 18, atk: 3, spd: 4, move: 2, ai: 'summoner',
    xp: 14, bounty: 12, abilities: ['vspam', 'vbite'],
    flavor: 'HOT SINGLES IN YOUR AREA want to summon pop-ups.'
  },
  miner: {
    name: 'Crypto Miner', hp: 20, atk: 2, spd: 3, move: 2, ai: 'miner',
    xp: 12, bounty: 20, abilities: ['vbite'],
    flavor: 'Ignores you. Steals 2 coins from the loot every turn it lives.'
  },
  captcha: {
    name: 'CAPTCHA Golem', hp: 55, atk: 8, spd: 4, move: 2, ai: 'melee', boss: true,
    xp: 40, bounty: 45, abilities: ['vbite', 'vverify'], captcha: true,
    flavor: 'SELECT ALL SQUARES CONTAINING YOUR DOOM. Invulnerable every other round.'
  },
  bsod: {
    name: 'B.S.O.D.', hp: 70, atk: 9, spd: 5, move: 3, ai: 'melee', boss: true,
    xp: 60, bounty: 70, abilities: ['vbsod', 'vbite', 'vsummon_drone'],
    flavor: 'Your PC ran into a problem: this guy. :('
  },
  spamking: {
    name: 'THE SPAM KING', hp: 90, atk: 10, spd: 6, move: 3, ai: 'summoner', boss: true,
...
DATA.VIRUS_ABILITIES = {
  vbite:    { name: 'Byte',        ico: 'bite',   cd: 0, range: 1, power: 1.0, desc: 'A byte-sized bite.' },
  vpoison:  { name: 'Infect',      ico: 'drop',   cd: 0, range: 1, power: 0.8, status: { id: 'poison', turns: 3, chance: 0.8 }, desc: 'Poisonous packet.' },
  vweb:     { name: 'Log Keys',    ico: 'web',    cd: 2, range: 3, power: 0.6, status: { id: 'slow', turns: 2, chance: 1 }, desc: 'Sticky spyware web.' },
  vsnipe:   { name: 'Data Snipe',  ico: 'crosshair', cd: 1, range: 4, power: 1.0, desc: 'Long-range packet loss.' },
  vencrypt: { name: 'Encrypt',     ico: 'lock',   cd: 3, range: 1, power: 1.1, status: { id: 'stun', turns: 1, chance: 0.7 }, steal: 4, desc: 'Locks a meme and demands coins.' },
  vphish:   { name: 'Phish Hook',  ico: 'hook',   cd: 2, range: 4, power: 0.7, pull: 2, desc: 'Reels a meme in. Click here!' },
  vspam:    { name: 'Spam Summon', ico: 'mail',   cd: 3, range: 0, summon: 'popup', count: 2, desc: 'You have (2) new pop-ups.' },
  vsummon_drone: { name: 'Botnet Call', ico: 'mail', cd: 4, range: 0, summon: 'drone', count: 1, desc: 'Calls a drone named Kevin.' },
  vbsod:    { name: 'Blue Screen', ico: 'window', cd: 4, range: 0, power: 0.9, aoe: 2, status: { id: 'stun', turns: 1, chance: 0.35 }, desc: 'CRITICAL_PROCESS_DIED for everyone nearby.' },
  vverify:  { name: 'Verify Human',ico: 'trafficlight', cd: 3, range: 2, power: 0.8, status: { id: 'confuse', turns: 1, chance: 0.6 }, desc: 'Identify all traffic lights or take damage.' },
  vroyalbonk: { name: 'Royal Decree', ico: 'can', cd: 2, range: 2, power: 1.4, desc: 'A can of spam, thrown with authority.' },
};

/* ============================================================
   MISSIONS — `ico` is a pixel-atlas name.
   ============================================================ */
DATA.MISSIONS = [
  {
    id: 'downloads', name: 'Downloads Folder', ico: 'doc', diff: 1,
    desc: 'Something free-robux-generator.exe shaped is moving in there.',
    foes: ['popup', 'popup', 'worm'],
    reward: [18, 28], itemChance: 0.35,
  },
  {
    id: 'recycle', name: 'Recycle Bin', ico: 'recycle', diff: 1,
    desc: 'The stuff you deleted is BACK and it is UNIONIZING.',
    foes: ['popup', 'worm', 'worm', 'blob'],
    reward: [25, 38], itemChance: 0.4,
  },
  {
    id: 'inbox', name: 'Email Swamp', ico: 'mail', diff: 2,
  ...
  {
    id: 'system32', name: 'System32', ico: 'window', diff: 3,
  ...
  {
    id: 'gpumines', name: 'The GPU Mines', ico: 'gpu', diff: 3,
  ...
  {
    id: 'darkweb', name: 'The Dark Web', ico: 'tinfoil', diff: 4,
  ...
  {
    id: 'spamfort', name: 'Spam Fortress', ico: 'can', diff: 5,
  ...
  {
    id: 'cloud', name: 'The Cloud (Endless)', ico: 'window', diff: 6, endless: true,
...
/* ============================================================
   PROGRESSION — systems reveal as you play (Mewgenics-style
   "you don't get everything at once"). `at` = trigger key.
   ============================================================ */
DATA.UNLOCKS = {
  shop:      { name: 'MemeBay',   ico: 'cart',  desc: 'A daily shop of hats, gear and consumables opened on your desktop.' },
  inventory: { name: 'Loot Stash',ico: 'bag',   desc: 'You can now hold loot, equip hats and use consumables.' },
  graveyard: { name: 'Graveyard', ico: 'grave', desc: 'Fallen memes are archived here — and can be necroposted back.' },
...
DATA.PHRASES = [
  ...
  'it is wednesday', 'bruh', 'much desktop', 'wow', 'ratio + L', 'skill issue', '4 universes ahead',
...
DATA.BATTLE_CRIES = ['LESGOOO', "IT'S MEMEING TIME", 'AVENGE ME', 'FOR THE DESKTOP!', 'no lag no lag', 'EZ Clap', 'I fear no update'];
